...
@section('content')
    ...
    <div class="row mb-4">
        <div class="col-md-6 col-xl-3 mb-3"><div class="zyga-kpi"><div class="kpi-label">Total</div><div class="kpi-value">{{ $totalProviders }}</div><div class="zyga-muted">Providers registrados</div><div class="kpi-icon"><i class="fas fa-users-cog"></i></div></div></div>
        <div class="col-md-6 col-xl-3 mb-3"><div class="zyga-kpi"><div class="kpi-label">Verificados</div><div class="kpi-value">{{ $verifiedProviders }}</div><div class="zyga-muted">Con validación administrativa</div><div class="kpi-icon"><i class="fas fa-shield-check"></i></div></div></div>
        <div class="col-md-6 col-xl-3 mb-3"><div class="zyga-kpi"><div class="kpi-label">Listos para operar</div><div class="kpi-value">{{ $readyProviders }}</div><div class="zyga-muted">Con servicios, horarios y documentos</div><div class="kpi-icon"><i class="fas fa-bolt"></i></div></div></div>
        <div class="col-md-6 col-xl-3 mb-3"><div class="zyga-kpi"><div class="kpi-label">Pendientes</div><div class="kpi-value">{{ $pendingProviders }}</div><div class="zyga-muted">Requieren acción administrativa</div><div class="kpi-icon"><i class="fas fa-user-clock"></i></div></div></div>
    </div>

    <div class="card">
        <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            <h3 class="zyga-section-title mb-2 mb-md-0">Listado de providers</h3>
                            ...
                            @foreach($providers as $provider)
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <div class="zyga-muted small">{{ $provider['user']['email'] ?? 'Sin correo asociado' }}</div>
                                    </td>
                                    <td>{{ $provider['provider_kind'] ?? '—' }}</td>
                                    <td><span class="badge badge-pill {{ $statusClass }}">{{ $statusName }}</span></td>
                                    <td><span class="badge badge-pill {{ $isVerified ? 'badge-soft-success' : 'badge-soft-warning' }}">{{ $isVerified ? 'Verificado' : 'Pendiente' }}</span></td>
                                    <td><span class="badge badge-pill {{ $isReady ? 'badge-soft-success' : 'badge-soft-danger' }}">{{ $isReady ? 'Listo para operar' : 'No listo' }}</span></td>
                                    <td class="text-right"><a href="{{ route('admin.providers.show', $provider['id']) }}" class="btn btn-sm btn-outline-primary">Abrir ficha</a></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
